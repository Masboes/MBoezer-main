      ...
      if(!this.paused){
        ...
        for(let i = 0; i < rows; i++) {
          for(let j = 0; j < cols; j++) {
            let angle = p.noise(i * step, j * step, p.frameCount * step * speed) * 2 * Math.PI;
            p.line(i * blocksize, j * blocksize, i * blocksize + Math.cos(angle) * blocksize * hairFactor, j * blocksize + Math.sin(angle) * blocksize * hairFactor);
          }
        }
      }
    }
